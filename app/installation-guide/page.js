...
// Original Desktop Step Component (100% Unchanged original parameters)
...
// Mobile Optimized Step Component (Clean vertical stacking variant)
const MobileStep = ({ num, title, children, visual }) => (
  <div style={{ padding: '2.5rem 0', borderBottom: '1px solid var(--border-light)', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
    <div>
      <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, borderRadius: '50%', background: 'var(--green)', color: 'white', fontFamily: 'var(--font-head)', fontWeight: 800, fontSize: '0.9rem', marginBottom: '0.75rem' }}>{num}</div>
      <h2 style={{ fontFamily: 'var(--font-head)', fontWeight: 800, fontSize: '1.35rem', color: 'var(--ink)', letterSpacing: '-0.02em', marginBottom: '0.75rem', lineHeight: 1.3 }}>{title}</h2>
      <div style={{ fontSize: '0.875rem', color: 'var(--muted)', lineHeight: 1.65 }}>{children}</div>
    </div>
    <div style={{ background: 'linear-gradient(135deg, rgba(10,173,110,0.06) 0%, rgba(10,173,110,0.12) 100%)', borderRadius: 16, border: '1px solid rgba(10,173,110,0.15)', padding: '1.5rem 1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 180 }}>
      <div style={{ width: '100%', maxWidth: '240px' }}>{visual}</div>
    </div>
  </div>
)
...
export default function InstallationGuide() {
  ...
  // Centralized asset matrix definitions to maintain matching layouts between views
  const timelineSteps = ['Choose Location', 'Mount on Pole', 'Connect to Router', 'Align Antenna', 'Test Signal']
  ...
  return (
    <>
      ...
      {/* OVERVIEW TIMELINE (Natively scroll-wrapped on mobile viewports) */}
      <div style={{ background: 'var(--green)', padding: '2rem 0', overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
        <div className="container" style={{ minWidth: 'max-content', width: '100%' }}>
      ...
      <section style={{ padding: '2rem 0 5rem', background: 'white' }}>
        <div className="container">
          ...
          {/* ── DESKTOP ONLY STRUCTURE (100% Unchanged original configuration layout) ── */}
          <div className="desktop-only">
            <Step num={1} title="Choose the Best Location" visual={getStepOneVisual()}>
            ...
            <Step num={2} title="Fix the Antenna on a Pole" visual={getStepTwoVisual()}>
            ...
            <Step num={3} title="Connect to Your Router" visual={getStepThreeVisual()}>
            ...
            <Step num={4} title="Align the Antenna for Best Signal" visual={getStepFourVisual()}>
            ...
            <Step num={5} title="If Signal is Weak — Troubleshoot" visual={getStepFiveVisual()}>
          ...
          {/* ── MOBILE ONLY STRUCTURE (Independent balanced vertical layouts) ── */}
          <div className="mobile-only">
            <MobileStep num={1} title="Choose the Best Location" visual={getStepOneVisual()}>
              <p>Make sure there is no obstruction between your antenna and the signal tower — no large trees, tall buildings, or metal structures. A clear, open line of sight is the single most important factor for strong signal reception.</p>
              <p style={{ marginTop: '0.5rem' }}>💡 <strong>Tip:</strong> Use your phone's signal to roughly identify the direction of the nearest tower. Walk around your property to find the spot with the most bars before mounting.</p>
            </MobileStep>

            <MobileStep num={2} title="Fix the Antenna on a Pole" visual={getStepTwoVisual()}>
              <p>Mount the antenna on a metal pole that is <strong>20–25 feet (6–8 metres) high</strong>. The higher the pole, the fewer obstructions and the stronger the signal. Clamp it securely using the U-bolts provided so it stays stable in wind and rain.</p>
              <div style={{ marginTop: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                {['Tighten all U-bolts firmly — the antenna must not rotate on its own', 'Run the cable neatly down the pole using cable ties', 'Leave a small drip loop at the bottom of the cable before entering the building'].map(t => (
                  <div key={t} style={{ display: 'flex', gap: '0.5rem', fontSize: '0.85rem', color: 'var(--slate)' }}>
            ...
            <MobileStep num={3} title="Connect to Your Router" visual={getStepThreeVisual()}>
              <p>Go to your wireless broadband router and follow these steps:</p>
              <ol style={{ paddingLeft: '1.2rem', marginTop: '0.5rem', display: 'flex', flexDirection: 'column', gap: '0.4rem', listStyle: 'decimal' }}>
                <li style={{ fontSize: '0.875rem', color: 'var(--slate)' }}>Unscrew and <strong>remove the small stick antennas</strong> currently on your router</li>
                <li style={{ fontSize: '0.875rem', color: 'var(--slate)' }}>Take the <strong>gold SMA Male connectors</strong> of the outdoor antenna cable</li>
                <li style={{ fontSize: '0.875rem', color: 'var(--slate)' }}>Screw them into the router's antenna ports — <strong>finger tight</strong>, then a quarter turn more</li>
              </ol>
              <div className="alert alert-info" style={{ marginTop: '1rem', padding: '0.75rem 1rem', fontSize: '0.82rem' }}>
                <span>💡 Most 4G routers have 2 antenna ports. Connect both cables for maximum MIMO gain.</span>
              </div>
            </MobileStep>

            <MobileStep num={4} title="Align the Antenna for Best Signal" visual={getStepFourVisual()}>
              <p>This step requires patience — it's the most important part of the installation:</p>
              <ol style={{ paddingLeft: '1.2rem', marginTop: '0.5rem', display: 'flex', flexDirection: 'column', gap: '0.4rem', listStyle: 'decimal' }}>
                <li style={{ fontSize: '0.875rem', color: 'var(--slate)' }}>Point the antenna roughly toward the signal tower</li>
                <li style={{ fontSize: '0.875rem', color: 'var(--slate)' }}><strong>Rotate the antenna slightly</strong> (5–10 degrees at a time)</li>
                <li style={{ fontSize: '0.875rem', color: 'var(--slate)' }}><strong>Wait 20 seconds</strong> for the router's signal bars to update</li>
                <li style={{ fontSize: '0.875rem', color: 'var(--slate)' }}>Check the signal bar count on your router</li>
                <li style={{ fontSize: '0.875rem', color: 'var(--slate)' }}>Repeat until you reach the <strong>highest signal bar count</strong></li>
                <li style={{ fontSize: '0.875rem', color: 'var(--slate)' }}><strong>Tighten the bracket</strong> firmly to lock the antenna in place</li>
              </ol>
            </MobileStep>

            <MobileStep num={5} title="If Signal is Weak — Troubleshoot" visual={getStepFiveVisual()}>
              <p>If you still don't get a good signal after alignment:</p>
              ...
              <div style={{ background: 'rgba(10,173,110,0.06)', border: '1px solid rgba(10,173,110,0.2)', borderRadius: 10, padding: '0.85rem', marginTop: '1rem', fontSize: '0.82rem', color: 'var(--slate)', lineHeight: 1.5 }}>
                📞 <strong>Still no signal?</strong> You may be eligible for our <strong>5-Day Checking Warranty</strong>. Send us a photo of your installation on <a href={`${WA_LINK}${encodeURIComponent('Hello! I need help with my antenna installation. Here is my photo:')}`} target="_blank" rel="noopener" style={{ color: 'var(--green)', fontWeight: 600 }}>WhatsApp +94 70 665 6007</a>
              </div>
            </MobileStep>

            {/* TIPS STACK MOBILE */}
            <div style={{ background: 'linear-gradient(135deg, rgba(10,173,110,0.06) 0%, rgba(10,173,110,0.12) 100%)', border: '1px solid rgba(10,173,110,0.2)', borderRadius: 16, padding: '1.5rem 1rem', marginTop: '2.5rem' }}>
              <h2 style={{ fontFamily: 'var(--font-head)', fontWeight: 800, fontSize: '1.25rem', color: 'var(--ink)', marginBottom: '1rem', textAlign: 'center' }}>💡 Tips for Best Results</h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {tipsData.map(t => (
                  <div key={t.tip} style={{ display: 'flex', gap: '0.5rem', padding: '0.85rem', background: 'white', borderRadius: 10, border: '1px solid var(--border-light)' }}>
                    <span style={{ fontSize: '1.1rem', flexShrink: 0 }}>{t.icon}</span>
                    <span style={{ fontSize: '0.82rem', color: 'var(--slate)', lineHeight: 1.5 }}>{t.tip}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* CTA (Symmetrical responsive padding targets) */}
      <section style={{ padding: '4rem 0', background: 'var(--bg)', textAlign: 'center' }}>
        <div className="container">
          ...
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href={`${WA_LINK}${encodeURIComponent('Hello! I need installation support for my Techo Connect Yagi Antenna.')}`} target="_blank" rel="noopener" className="btn btn-wa btn-lg" style={{ width: '100%', maxWidth: '320px' }}>
            ...
            <Link href="/products" className="btn btn-outline btn-lg" style={{ width: '100%', maxWidth: '320px', textAlign: 'center' }}>Shop Our Antennas</Link>
          </div>
        </div>
      </section>
